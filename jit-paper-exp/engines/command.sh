...
function get_engine_cmd() {
    ...
    case $engine in
        ...
        "wazero")
            ...
            ;;
        "wasmer")
            echo $(find_binary "$WASMER" wasmer-link wasmer) $ENGINE_OPTIONS run --disable-cache
            ;;
        "wasmer-base")
            echo $(find_binary "$WASMER" wasmer-link wasmer) $ENGINE_OPTIONS run --singlepass --disable-cache
            ;;
        "wavm")
